const Document  = require("../models/commercial-document.model");
const DeleteOtp = require("../../stock/models/delete-otp.model");

const commercialDocumentService = {
  async getAll() {
  ...
  async create({ originalName, mimeType, size, data, description, uploadedBy }) {
    ...
    return new Document({ originalName, mimeType, size, data: buffer, description, uploadedBy }).save();
  },
  ...
  async getStats() {
    ...
    return { total, monthCount, totalSize: sizeResult[0]?.totalSize ?? 0 };
  },
};

module.exports = commercialDocumentService;
