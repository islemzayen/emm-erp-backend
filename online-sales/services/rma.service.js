...
// Creates an RMA in PENDING status immediately when an online return is submitted,
// before the return is approved or rejected. Skips the delivery-status guard because
// the online order may not be linked to a commercial SalesOrder yet.
exports.createPending = async ({ salesOrderId, lines, notes = "", createdBy = null }) => {
  const order = await SalesOrder.findById(salesOrderId).select("orderNo customerName");
  if (!order) {
    throw Object.assign(new Error("Sales order not found"), { statusCode: 404 });
  }
...
exports.authorizeRma = async (id, documentId = null) => {
  const rma = await RMA.findById(id);
  if (!rma) throw Object.assign(new Error("RMA not found"), { statusCode: 404 });

  rma.rmaCase = "AUTHORIZED";
  if (documentId) rma.rmaDocumentRef = documentId;
  // Return accepted → it's no longer "pending", enter the normal RMA flow
  if (rma.status === "PENDING") rma.status = "OPEN";
  await rma.save();
  return populateRma(RMA.findById(rma._id));
};

exports.rejectRma = async (id) => {
  const rma = await RMA.findById(id);
  if (!rma) throw Object.assign(new Error("RMA not found"), { statusCode: 404 });

  rma.rmaCase = "NOT_AUTHORIZED";
  rma.status  = "REJECTED";
  await rma.save();
  return populateRma(RMA.findById(rma._id));
};
